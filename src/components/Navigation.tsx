...
const Navigation = () => {
  ...
  return <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'p-2' : ''}`}>
      <div className={`transition-all duration-300 bg-off-white/95 backdrop-blur-sm ${isScrolled ? 'max-w-screen-2xl mx-auto rounded-xl shadow-medium border border-light-gray/50' : 'border-b border-black/10'}`}>
        <div className={`flex items-center justify-between h-16 ${isScrolled ? 'px-6 lg:px-8' : 'max-w-screen-2xl mx-auto px-6 lg:px-8'}`}>
          {/* Logo */}
